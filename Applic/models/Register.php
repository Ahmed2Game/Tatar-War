...
class Register_Model extends Model
{
    ...
    public function isPlayerNameExists( $playerName, $playerEmail )
    {
        ...
        $email = 0 < db2::get_field( "SELECT COUNT(*) FROM p_players p WHERE p.email=:playerEmail", array(
            'playerEmail' => $playerEmail
        ) );
		
		$name = 0 < db2::get_field( "SELECT COUNT(*) FROM p_players p WHERE p.name=:playerName", array(
            'playerName' =>  $playerName
        ) );
		return array(
		    'master' => array('name' => $name, 'email' => $email),
		);
    ...
    public function createNewPlayer( $playerId, $playerName, $tribeId, $position, $villageName, $mapSize, $playerType, $villageCount = 1, $playerIp = NULL, $playerInvite = 0, $fid = 3 )
    {
        ...
        if ( $expr != "" )
        {
            ...
            global $serv;
			db2::query( "UPDATE servers SET players_count=players_count+1 WHERE id=:id", array('id' => $serv));
        }
