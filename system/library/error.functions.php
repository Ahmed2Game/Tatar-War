...
function log_error($html)
{
    ...
    if (isset($_COOKIE['server'])) {
        ...
        $detail = $_COOKIE['server'] . "-" . $cookie->uname;
    } else {
        $detail = null;
    }
    ...
    if ($GLOBALS['error_n'] == 1)
        $html = (!file_exists($file) ? "{$date} {$_SERVER['SERVER_SOFTWARE']} <br/>\n" : null) . "<br/><br/>\n\n" . "<a name=\"{$detail}\"></a>#{$GLOBALS['error_time']} {$_SERVER['REQUEST_URI']}" . ($_SERVER['REQUEST_METHOD'] == 'POST' ? "POST: <br/>\n<pre>" . print_r($_POST, true) : null) . "</pre><br/><br/>\n\n" . $html;
